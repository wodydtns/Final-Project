...
<div >
	<div class="card">
		<div class="card-header">사전조사 신청 리스트</div>
		<div class="card-body">
...
var searchForm = $("#searchForm").on("submit", function(event){
	...
	var action="${cPath }/master/adminClassList.do";
	$.ajax({
		url:  action,
		method : "get",
		data : queryString,
...
var permitForm = $("#permitForm");
$(document).on("click","[name='deny']",function(){
	var denyVal = $(this).val();
	var action = "${cPath}/master/deny.do";
	permitForm.find("[name='permit']").val(denyVal);
	permitForm.attr("action",action);
	permitForm.submit();
});
$(document).on("click","[name='accept']",function(){
	
	var acceptVal = $(this).val();
	var action = "${cPath}/master/accept.do";
	permitForm.find("[name='permit']").val(acceptVal);
	permitForm.attr("action",action);
	permitForm.submit();
});
</script>
