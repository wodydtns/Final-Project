...
* 수정일                 수정자      수정내용
* ----------  ---------  -----------------
* 2020. 3. 20.      최효은      최초작성, 회원별 작업 리스트 출력
...
<div class="container-fluid">
	...
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
		<div>
			<h1 class="h3 mb-2 text-gray-800">작업 리스트</h1><br>
			<p class="mb-4">프로젝트의 작업 리스트가 표시됩니다.</p>
		</div>
		<a href="<c:url value='/pmsProject/projectWorkInsert.do' />" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" style="color:white;">
		<i class="fas fa-arrow-alt-circle-right fa-sm text-white-50"></i> 새 작업 등록</a>
	</div>
	...
	<div class="card shadow mb-4">
		<div class="card-header py-3">
			...
			<div id="inputForm" class="form-inline justify-content-center mb-3">
				<select name="mem_nick" class="pms-input-border">
					<!-- <spring:message code="proj.proj_cate" /> -->
					<option value="${pwList['mem_nick']}">
					${not empty pwList['mem_nick']?pwList['mem_nick']:"담당자 선택"}</option>
					<c:forEach items="${projectMemberList }" var="pmList">
						<option value="${pmList['mem_nick'] }">${pmList['mem_nick'] }</option>
					</c:forEach>
				</select>
...
var searchForm = $("#searchForm").on("submit", function(event){
	...
	let method = $(this).attr("method");
		// 비동기 처리 요청
		$.ajax({
			url: "<c:url value='/pmsProject/projectWorkList.do' />",
			method: method,
			data : queryString,
			dataType : "json", 
				...
				if(dataList.length>0){
					$(dataList).each(function(index, pwList){
						...
						eachTr.append(
							...
							$("<td>").html(
								$("<a>").attr({
									"href" : "<c:url value='/pmsProject/projectWorkView.do'/>?what="+pwList.pw_cd
