...
var searchForm = $("#searchForm").on("submit", function(event){
	...
	if(!action) action = "<c:url value='/hobby/hobbyCategory.do' />";
	if(!method) method = "get";
		
	let addr = "<c:url value='/hobby/hobbyDetail.do' />";
	$.ajax({
		url: "<c:url value='/hobby/hobbyCategory.do'/>",
		method: "get",
		data: queryString,
		dataType: "json",
			...
			if(dataList.length>0){
				$(dataList).each(function(index, category){
					...
					eachTr.append(
							...
							$("<tr>").html(
									$("<td>").text(
											"클래스 시작일 : "+category.cl_start)	
							),
							$("<tr>").html(
									$("<td>").text(
											"클래스 종료일 : "+category.cl_end )	
							),
							$("<tr>").html(
								$("<td>").text(
										"클래스 가격 : "+category.cl_fee)		
							),
							$("<tr>").html(
								$("<td>").text(
										"좋아요 수 : "+category.cl_like)	
							)
					);
					trTags.push(eachTr);
				});
